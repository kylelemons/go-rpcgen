...
func (p *Plugin) GenerateRPCStubs(svc *descriptor.ServiceDescriptorProto) {
	...
	for _, m := range svc.Method {
		...
		p.P("func (c rpc", name, "Client) ", method, "(in *", p.TypeName(iType), ", out *", p.TypeName(oType), ") error {")
		p.In()
	...
	p.P("// ", name, " remotely.")
	p.P("func New", name, "Client(addr string) ", name, " {")
	p.In()
	p.P("return rpc", name, "Client{client.NewClient(addr)}")
	p.Out()
	p.P("}")
	p.P()
