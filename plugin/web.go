...
func (p *Plugin) GenerateWebStubs(svc *descriptor.ServiceDescriptorProto) {
	...
	for _, m := range svc.Method {
		...
		p.In()
		p.P(`return webrpc.Post(this.remote, "/`, name, "/", method, `", in, out)`)
		p.Out()
		p.P("}")
		p.P()
	}
	...
	for _, m := range svc.Method {
		...
		p.P(`if err := c.ReadProto(in); err != nil {`)
		p.In()
		p.P(`return err`)
		p.Out()
		p.P(`}`)
	...
	p.P("func New", name, "WebClient(remote *url.URL) ", name, " {")
	p.In()
	p.P("return rpc", name, "WebClient{remote}")
	p.Out()
	p.P("}")
}
